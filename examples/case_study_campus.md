...
# PhantomTap Attack-Path Analysis

**Crown jewel:** `datacenter` · **Entry:** `outside`  
**Path of least resistance (breach cost 141):** `outside` → `lobby` → `west-wing` → `datacenter`
**Doors on that path:** `front-door`, `lobby-west`, `west-core`

## Harden-first chokepoints

Hardening *one* door raises the intruder's cheapest path by:

| Door | Door risk | +Path cost if hardened | On current path |
|------|----------:|-----------------------:|:---------------:|
| `front-door` | 75 | +15 | ✓ |
| `lobby-west` | 62 | +6 | ✓ |
| `west-core` | 22 | +6 | ✓ |
...
**Harden `front-door` first** — it buys the biggest increase (+15) in the cost of reaching `datacenter`, which is often *not* the estate's weakest door in isolation.