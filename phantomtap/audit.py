...
def quick_risk_score(dep: Deployment) -> int:
    """A fast composite risk estimate that skips the expensive characterization
    loop (re-weighting the remaining factors to sum to 1). Used for what-if
    remediation analysis where many variants must be scored cheaply.
    """
    factors = [
        _format_factor(dep),
    ...
    total_w = sum(WEIGHTS[f.factor] for f in factors)
    composite = sum(WEIGHTS[f.factor] * f.score for f in factors) / total_w
...
def render_markdown(result: AuditResult, dep: Optional[Deployment] = None) -> str:
    ...
    if dep is not None:
        from .remediation import prioritized_plan
        plan = prioritized_plan(dep)
        if plan:
            lines.append("")
            lines.append("## Prioritized remediation roadmap")
            lines.append("")
            lines.append("Highest-leverage fix first; risk is the projected "
                         "composite *after* applying each step in sequence.")
            lines.append("")
            lines.append("| Step | Fix | Risk after | Δ |")
            lines.append("|-----:|-----|-----------:|---:|")
            for i, f in enumerate(plan, 1):
                lines.append(f"| {i} | {f.action} | {f.new_risk} | "
                             f"−{f.delta} |")
            lines.append("")
            if plan:
                lines.append(f"Applying the full roadmap moves this deployment "
                             f"to a projected **{plan[-1].new_risk}/100**.")
    lines.append("")
    lines.append("---")
